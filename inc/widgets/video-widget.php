...
// Start class
if ( ! class_exists( 'WPSP_Video_Widget' ) ) {
	class WPSP_Video_Widget extends WP_Widget {
		...
		public function __construct() {
			...
			parent::__construct(
				'wpsp-video-widget',
	            $branding . esc_html__( 'Video', 'wpsp_widget' ),
	            $widget_ops = array(
	                'classname'         => 'wpsp-video-widget',
	            )
			);
		}
		...
		function widget( $args, $instance ) {
			...
			echo $args['before_widget'];
				...
				if ( $description ) {
					echo '<div class="wpsp-video-widget-description">'. wpsp_sanitize_data( $description, 'html' ) .'</div>';
				}
		...
		function update( $new_instance, $old_instance ) {
			...
			$instance['video_description'] = ! empty( $new_instance['video_description'] ) ? wpsp_sanitize_data( $new_instance['video_description'], 'html' ) : '';
			return $instance;
		}
		...
		function form( $instance ) {
			...
			<p>
				...
				<textarea rows="5" class="widefat" name="<?php echo esc_attr( $this->get_field_name( 'video_description' ) ); ?>" type="text"><?php echo wpsp_sanitize_data( $instance['video_description'], 'html' ); ?></textarea>
			</p>
...
// Register the widget
if ( ! function_exists( 'register_wpsp_video_widget' ) ) {
	function register_wpsp_video_widget() {
		register_widget( 'WPSP_Video_Widget' );
	}
}
add_action( 'widgets_init', 'register_wpsp_video_widget' );